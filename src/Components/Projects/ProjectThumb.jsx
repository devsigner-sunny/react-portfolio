      ...
      <Parallax speed={-5} translateX={parallaxMoveValue}>
        <a
         ...
         rel="noopener noreferrer" 
          ...
          <ImageEffect imgSrc={image} className='absolute -translate-x-1/2 opacity-0 -translate-y-2/3 top-1/2 left-1/2' />
          <h2 className="relative flex uppercase text-8xl">{title}<VscLinkExternal className="pt-2 mt-2 text-3xl" /></h2>
          <div className="keyword-list">
            <div className="keyword-list-container">
                {keywords.map((keyword, index) => (
                <span
                  key={index}
                  className="px-2 keyword text__primary"
                >
                  #{keyword}
                </span>
              ))}
